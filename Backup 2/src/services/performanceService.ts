...
export const performanceService = {
  ...
  async updateUserKPIMapping(id: string, updates: Partial<UserKPIMapping>): Promise<UserKPIMapping> {
    const { data, error } = await supabase
      .from('user_kpi_mappings')
      .update(updates)
      .eq('id', id)
  ...
  async getEffectiveTargetsForUser(teamMemberId: string): Promise<EffectiveTarget[]> {
    try {
      const { data, error } = await supabase.rpc('get_effective_targets_for_user', {
        p_team_member_id: teamMemberId
      });
      
      if (error) {
        console.warn('Could not get effective targets from function, falling back to manual logic:', error);
        return this.getEffectiveTargetsManual(teamMemberId);
      }
      
      return data || [];
    } catch (error) {
      console.warn('Function not available, using manual target resolution:', error);
      return this.getEffectiveTargetsManual(teamMemberId);
    }
  },

  async getEffectiveTargetsManual(teamMemberId: string): Promise<EffectiveTarget[]> {
    // Get user-specific mappings
    const userMappings = await this.getUserKPIMappings(teamMemberId);
    
    // Get team member's designation
    ...
    // Get designation-level targets
    const designationTargets = await this.getKPITargetsByRole(teamMember.designation);
    
    // Create effective targets map
    const effectiveTargets: { [key: string]: EffectiveTarget } = {};
    
    // Start with designation defaults
    designationTargets.forEach(target => {
      effectiveTargets[target.kpi_name] = {
    ...
    // Override with user-specific mappings
    userMappings.forEach(mapping => {
      effectiveTargets[mapping.kpi_name] = {
    ...
    return Object.values(effectiveTargets);
  },
